 ...
 */
 ...
 export default class JoyStick extends ui.RockerUI {
     /**最大滑动距离（超过距离则显示操纵杆） */
     ...
     public static _isTouchMove: Boolean = false;
     /**摇杆的角度 */
     public static angle: number = -1;
     /**摇杆的弧度 */
     public static radians: number = -1;

    //  public joystickPoint: Laya.Image

 
 
     constructor(touchSp: Laya.Sprite) {//touchSp: Laya.Sprite
         super();
     ...
     private _onMouseMove(evt: Event): void {
        ...
        //  } else {
             ...
             if (JoyStick.angle < 0) JoyStick.angle += 360;
             //对角度取整
             ...
             if (dx + dy >= this._joystickRadius * this._joystickRadius) {
                 //控制点在半径的位置（根据弧度变化）
                 let x: number = Math.floor(Math.sin(JoyStick.radians) * this._joystickRadius + this._originPiont.x);
                 let y: number = Math.floor(Math.cos(JoyStick.radians) * this._joystickRadius + this._originPiont.y);
